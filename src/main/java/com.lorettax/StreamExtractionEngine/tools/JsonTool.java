...
public class JsonTool {
	...
	public static <T> T getValueByPath(JSONObject jsonObject, String jsonPath, Class<T> tClass) {
		Preconditions.checkArgument(StringUtils.isNotBlank(jsonPath), "jsonPath is black");
		...
		try {
			...
			throw new RuntimeException(String.format("failed to eval jsonObject[%s], jsonPath[%s], tClass[%s]", jsonObject, jsonPath, tClass));
		}
		if (obj == null) {
			return null;
		}
