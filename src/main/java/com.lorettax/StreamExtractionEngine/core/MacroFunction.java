...
public class MacroFunction {
    ...
    public boolean equals(Object o) {
        ...
        if (argsSize != macroFunction.argsSize) {
            return false;
        }
    ...
    public static MacroFunction parseMacro(String name, String value) {
        LinkStreamFQLParser parser = new LinkStreamFQLParser();
        Map<LinkStreamFQL, Integer> featureDSLMap = parser.parse(name);
        if (featureDSLMap.size() != 1) {
            throw new IllegalArgumentException(String.format("invalid macro, name[%s], value[%s]", name, value));
        }
        LinkStreamFQL nameDSL = featureDSLMap.keySet().toArray(new LinkStreamFQL[0])[0];
    ...
    public List<String> replace(List<String> value, Map<String, List<String>> functionTokenMap) {
        ...
        LinkStreamFQLParser parser = new LinkStreamFQLParser();
        Map<String, List<String>> functionTokens = new HashMap<String, List<String>>();
