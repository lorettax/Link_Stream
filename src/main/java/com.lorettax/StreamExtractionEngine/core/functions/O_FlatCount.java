...
public class O_FlatCount extends AbstractFunction {
    ...
    public Map<String, Object> doExecute(LinkStreamFQL fql,
		...
		if(isUpdateMode(mode)) {
			...
			if(!isNotNull(target)) {
				if (!LinkStreamFQLResult.class.isInstance(target)) {
				...
				LinkStreamFQLResult linkStreamFQLResult = (LinkStreamFQLResult) target;
				Field lastTarget = linkStreamFQLResult.getFql().getTarget();
		...
		if(isGetMode(mode)) {
			...
			if(isNull(target)) {
                ...
                LinkStreamFQLResult linkStreamFQLResult = (LinkStreamFQLResult) target;
                Object lastResult = linkStreamFQLResult.getResult();
