...
import com.lorettax.StreamExtractionEngine.tools.TimeoutUtils;
...
public class LinkStreamFQLExecuteEngine {

    private static final Logger logger = LoggerFactory.getLogger(LinkStreamFQLExecuteEngine.class);
    ...
    private CompletableFuture<JSONObject> executeAsync(Map<String, Set<LinkStreamFQL>> dslTree,
                                                       JSONObject event,
                                                       Map<String, Object> helper,
                                                       String mode,
                                                       String depth,
                                                       Map<LinkStreamFQL, CompletableFuture<Map<String, Object>>> functionFuturesContainer) {
        Map<LinkStreamFQL, CompletableFuture<Map<String, Object>>> currentDepthFunctionFutures = new HashMap<>();
        for(LinkStreamFQL function : dslTree.get(depth)) {
        ...
        CompletableFuture<JSONObject> result = allFutures.thenApply(v -> {
            ...
            for(Map.Entry<LinkStreamFQL, CompletableFuture<Map<String, Object>>> entry :
                    currentDepthFunctionFutures.entrySet()) {
                LinkStreamFQL function = entry.getKey();
    ...
    private Map<String, Object> execute(LinkStreamFQL fql,
                                        JSONObject event,
                                        Map<String, Object> helper,
                                        String mode) throws Exception {
        return LinkStreamFQLRegistry.getFunction(fql.getOp()).execute(fql, event, helper, mode);
    }



    public CompletableFuture<JSONObject> executeTreeAsync(Map<String, Set<LinkStreamFQL>> dslTree,
                                                          JSONObject event,
                                                          Map<String, Object> helper,
                                                          String mode) {
        ...
        Map<LinkStreamFQL, CompletableFuture<Map<String, Object>>> functionFuturesContainer = new HashMap<>();
        ...
        CompletableFuture<Map<LinkStreamFQL, Map<String, Object>>> allHaveDone = resultFuture
                .applyToEither(timer, v -> {
                    if (!timer.isDone()) {
                        timer.cancel(true);
                    }

                    List<Map.Entry<LinkStreamFQL, CompletableFuture<Map<String, Object>>>> notDone =
                    ...
                    List<Map.Entry<LinkStreamFQL, CompletableFuture<Map<String, Object>>>> done =
                            functionFuturesContainer.entrySet().stream()
                                    .filter(e -> e.getValue().isDone()).collect(Collectors.toList());

                    final Map<LinkStreamFQL, Map<String, Object>> doneFunctionResults = new HashMap<>();
    ...
    public JSONObject executeTree(Map<String, Set<LinkStreamFQL>> dslTree,
                                  JSONObject event,
