...
import com.lorettax.StreamExtractionEngine.tools.Base64Utils;
...
public class LinkStreamFQLParser {
    private static final Logger logger = LoggerFactory.getLogger(LinkStreamFQLParser.class);
    ...
    public Map<String, Set<LinkStreamFQL>> parseExecutionTree(List<String> dsls) {
    ...
    public String parseFromTokens(
            ...
            Map<String, LinkStreamFQL> featureDSLMap,
            Map<LinkStreamFQL, Integer> functions) {
        ...
        List<Tuple2<LinkStreamFQL, Integer>> functionList = new LinkedList<>();
        ...
        for (String token : tokens) {
            if (")".equals(token)) {
                ...
                LinkStreamFQL newFunction = genFunctionDSL(applicationSetting, newFunctionTokens);
        ...
        for (Tuple2<LinkStreamFQL, Integer> tuple : functionList) {
    ...
    public PreParseInfo preParse(Map<MacroFunction, MacroFunction> macros, String dsl, boolean dslNorMalized) {
        ...
        Map<String, LinkStreamFQL> featureDSLMap = new HashMap<>();
    ...
    public Map<String, Set<LinkStreamFQL>> parseExecutionTree(JSONObject applicationSetting,
                                                              List<String> dsls,
        ...
        // parse dsl
        Map<String, Set<LinkStreamFQL>> result = new HashMap<>();
        Map<LinkStreamFQL, Tuple2<LinkStreamFQL, Integer>> allFunctions = new HashMap<>();
        for (String elem : dsls) {
            PreParseInfo preParseInfo = preParse(macros, elem, dslNormalized);
            Map<LinkStreamFQL, Integer> functions = new HashMap<>();
            Map<String, LinkStreamFQL> featureDSLMap = new HashMap<>();
            ...
            LinkStreamFQL topFQL = featureDSLMap.get(topFunction);
            if (!topFQL.getName().equals(preParseInfo.name)) {
                topFQL.getAlias().add(preParseInfo.text_name);
            }

            for (Map.Entry<LinkStreamFQL, Integer> entry : functions.entrySet()) {
                Tuple2<LinkStreamFQL, Integer> oldFeatureDSL = allFunctions.get(entry.getKey());
        ...
        for (Tuple2<LinkStreamFQL, Integer> entry : allFunctions.values()) {
    ...
    public Map<LinkStreamFQL, Integer> parse(String dsl) {
    ...
    public Map<LinkStreamFQL, Integer> parse(JSONObject applicationSetting, String dsl, boolean dslNormalized) {
        Map<LinkStreamFQL, Integer> functions = new HashMap<>();
        List<String> tokens = parseTokens(dsl, dslNormalized);
    ...
    private LinkStreamFQL genFunctionDSL(JSONObject applicationSetting, List<String> newFunctionTokens) {
        ...
        LinkStreamFQL fql = new LinkStreamFQL();
        fql.setOp(op);
        if (op.startsWith("M_")) {
            fql.setWindow(null);
